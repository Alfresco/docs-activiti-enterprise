---
Title: BPMN elements
---

# BPMN elements
All BPMN elements have a `name` and `documentation` property. The documentation property is a free text description of the element within your process. 

The BPMN elements available in Alfresco Activiti Enterprise are: 

-   [Start events](../processes-bpmn/bpmn-start.md)
	-   [Error start events](../processes-bpmn/bpmn-start.md#error-start-events)
	-   [Message start events](../processes-bpmn/bpmn-start.md#message-start-events)
	-   [Start events](../processes-bpmn/bpmn-start.md#start-events)
	-   [Signal start events](../processes-bpmn/bpmn-start.md#signal-start-events) 
	-   [Timer start events](../processes-bpmn/bpmn-start.md#timer-start-events)
-   [Boundary events](../processes-bpmn/bpmn-boundary.md)
	-   [Error boundary events](../processes-bpmn/bpmn-boundary.md#error-boundary-events)
	-   [Message boundary events](../processes-bpmn/bpmn-boundary.md#message-boundary-events)   
	-   [Signal boundary events](../processes-bpmn/bpmn-boundary.md#signal-boundary-events)
	-   [Timer boundary events](../processes-bpmn/bpmn-boundary.md#timer-boundary-events)
-   [Error events](../processes-bpmn/bpmn-error.md)
-   [Message events](../processes-bpmn/bpmn-message.md)
	-	[Message intermediate throw events](../processes-bpmn/bpmn-message.md#message-intermediate-throw-events) 
	- [Message intermediate catch events](../processes-bpmn/bpmn-message.md#message-intermediate-catch-events)  
-   [Timer events](../processes-bpmn/bpmn-timer.md)
	-   [Timer intermediate catch events](../processes-bpmn/bpmn-timer.md#timer-intermediate-catch-events)
-   [Signal events](../processes-bpmn/bpmn-signal.md)
	-   [Signal intermediate throw events](../processes-bpmn/bpmn-signal.md#signal-intermediate-throw-events)
	-   [Signal intermediate catch events](../processes-bpmn/bpmn-signal.md#signal-intermediate-catch-events)
-   [Gateways](../processes-bpmn/bpmn-gateways.md)
	-   [Exclusive gateways](../processes-bpmn/bpmn-gateways.md#exclusive-gateways)
	-   [Inclusive gateways](../processes-bpmn/bpmn-gateways.md#inclusive-gateways)
	-   [Parallel gateways](../processes-bpmn/bpmn-gateways.md#parallel-gateways)
-   [Business rule tasks](../processes-bpmn/bpmn-business.md)
-   [Call activities](../processes-bpmn/bpmn-call.md)
-   [Service tasks](../processes-bpmn/bpmn-service.md)
-   [Sub-processes](../processes-bpmn/bpmn-sub.md)
	-	 [Expanded and collapsed sub-processes](../processes-bpmn/bpmn-sub.md##expanded-and-collapsed-sub-processes)
	-	 [Event sub-processes](../processes-bpmn/bpmn-sub.md#event-sub-processes)   
-   [User tasks](../processes-bpmn/bpmn-user.md)
-   [End events](../processes-bpmn/bpmn-end.md)
	-   [End events](../processes-bpmn/bpmn-end.md#end-events)
	-   [Error end events](../processes-bpmn/bpmn-end.md#error-end-events)
	-   [Message end events](../processes-bpmn/bpmn-end.md#message-end-events)
	-   [Terminate end events](../processes-bpmn/bpmn-end.md#terminate-end-events)   
-   [Sequence flows](../processes-bpmn/bpmn-sequence.md)